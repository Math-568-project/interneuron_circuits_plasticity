...
def plot_alltuningcurves(tunings_initial,
                         tuning_rewardend,
    ...
    if save is not None:
        plt.savefig('%s/singletuning%s.eps' % (save, name),
                    bbox_extra_artists=(lgd, ),
                    bbox_inches='tight')


def plot_responsechangehistogram(tunings_initial,
                                 tunings_final,
                                 N_neurons,
                                 N_pop,
                                 ax,
    ...
    for k in range(0, N_neurons, int(groupsize)):
        ttest = stats.ttest_ind(tunings_initial[k:int(k + groupsize), 0],
                                tunings_final[k:int(k + groupsize), 0])
        pvalues.append(ttest[1])
        initial_means.append(
            np.mean(tunings_initial[k:int(k + groupsize), 0], 0))
    ...
    if save is not None:
        plt.tight_layout()
        plt.savefig('%s/barplot%s.pdf' % (save, name),
                    bbox_extra_artists=(lgd, ),
                    bbox_inches='tight')


def plot_SOMswitch(tunings_initial,
                   tunings_final,
                   N_neurons,
                   N_pop,
                   save=None,
    ...
    plt.tight_layout()
    plt.savefig('%s/SOMswitch%s.pdf' % (save, name),
                bbox_extra_artists=(lgd, ),
                bbox_inches='tight')
...
def plot_tuningcurves(tunings_initial,
    ...
    if save is not None:
        plt.savefig('%s/tuning%s.eps' % (save, name),
                    bbox_extra_artists=(lgd, ),
                    bbox_inches='tight')


def plot_grandavgtuningcurves(tunings_initial,
                              tunings_final,
                              N_pop,
                              save,
                              name=''):
    ...
    for k in range(0, N_pop):
        ...
        for m in range(np.shape(tunings_initial)[0]):
            plt.plot(np.arange(np.shape(tunings_initial)[2]),
                     tunings_initial[m, k:k + 1, :].T,
                     '.7',
                     lw=.5)
        plt.plot(np.arange(np.shape(tunings_initial)[2]),
                 np.mean(tunings_initial[:, k:k + 1, :], 0).T,
                 'k',
                 lw=2)

        for m in range(np.shape(tunings_initial)[0]):
            plt.plot(np.arange(np.shape(tunings_initial)[2]),
                     tunings_final[m, k:k + 1, :].T,
                     color=cmaps.viridis(0.55),
                     lw=.5)
        plt.plot(np.arange(np.shape(tunings_initial)[2]),
                 np.mean(tunings_final[:, k:k + 1, :], 0).T,
                 color=cmaps.viridis(0.6),
                 lw=2)
    ...
    plt.savefig('%s/tuninggrandavg%s.eps' % (save, name),
                bbox_extra_artists=(lgd, ),
                bbox_inches='tight')
...
def plot_firingratechange(tunings_initial, tunings_final, save, name=''):
    ...
    plt.scatter(tunings_initial[:, 0],
                tunings_final[:, 0],
                color=cmaps.viridis(0.0),
                lw=1)
    plt.scatter(tunings_initial[:, 1],
                tunings_final[:, 1],
                color=cmaps.viridis(0.33),
                lw=1)
    plt.scatter(tunings_initial[:, 2],
                tunings_final[:, 2],
                color=cmaps.viridis(0.66),
                lw=1)
    plt.scatter(tunings_initial[:, 3],
                tunings_final[:, 3],
                color=cmaps.viridis(1.0),
                lw=1)
    plt.plot(np.arange(-.1, max_rate + .2, .1),
             np.arange(-.1, max_rate + .2, .1),
             color='k')
    ...
    plt.savefig(
        '%s/firingratechange%s.eps' %
        (save, name))  #, bbox_extra_artists=(lgd,), bbox_inches='tight')
...
def plot_currentratiochange(current_initial, current_final, save, name=''):
    ...
    plt.scatter(current_initial[:, 0],
                current_final[:, 0],
                color=cmaps.viridis(0.0),
                lw=1)
    plt.scatter(current_initial[:, 1],
                current_final[:, 1],
                color=cmaps.viridis(0.33),
                lw=1)
    plt.scatter(current_initial[:, 2],
                current_final[:, 2],
                color=cmaps.viridis(0.66),
                lw=1)
    plt.scatter(current_initial[:, 3],
                current_final[:, 3],
                color=cmaps.viridis(1.0),
                lw=1)
    plt.plot(np.arange(1, max_current + .2, .1),
             np.arange(1, max_current + .2, .1),
             color='k')
...
def plot_currentchange(current_initial, current_final, save, name=''):
    ...
    if min_current > 0:
        min_current = 0
    plt.plot(np.arange(min_current - .1, max_current + .2, .1),
             np.arange(min_current - .1, max_current + .2, .1),
             color='k')
    if name == 'E':
    ...
    plt.savefig(
        '%s/currentchange%s.eps' % (save, name)
    )  #, bbox_extra_artists=(lgd,), bbox_inches='tight')#, bbox_extra_artists=(lgd,), bbox_inches='tight')
...
def rasterplots(neuront, neuroni, timeframe, start, end, name):
    ...
    if name == 'PYR':
        plot(neuront[neuroni < 100] / second,
             neuroni[neuroni < 100],
             '.',
             color=cmaps.viridis(0.0),
             ms=2.0)
        plot(neuront[((neuroni >= 100) & (neuroni < 200))] / second,
             neuroni[((neuroni >= 100) & (neuroni < 200))],
             '.',
             color=cmaps.viridis(0.33),
             ms=2.0)
        plot(neuront[((neuroni >= 200) & (neuroni < 300))] / second,
             neuroni[((neuroni >= 200) & (neuroni < 300))],
             '.',
             color=cmaps.viridis(0.66),
             ms=2.0)
        plot(neuront[neuroni >= 300] / second,
             neuroni[neuroni >= 300],
             '.',
             color=cmaps.viridis(1.0),
             ms=2.0)
        ...
        plot(neuront[neuroni < 30] / second,
             neuroni[neuroni < 30],
             '.',
             color=cmaps.viridis(0.0),
             ms=2.0)
        plot(neuront[((neuroni >= 30) & (neuroni < 60))] / second,
             neuroni[((neuroni >= 30) & (neuroni < 60))],
             '.',
             color=cmaps.viridis(0.33),
             ms=2.0)
        plot(neuront[((neuroni >= 60) & (neuroni < 90))] / second,
             neuroni[((neuroni >= 60) & (neuroni < 90))],
             '.',
             color=cmaps.viridis(0.66),
             ms=2.0)
        plot(neuront[neuroni >= 90] / second,
             neuroni[neuroni >= 90],
             '.',
             color=cmaps.viridis(1.0),
             ms=2.0)
    ...
    plt.savefig(
        '%s/%s/%s/%srasterplot%d.pdf' %
        (savepath, dataname, run_no, name, timeframe),
        rasterized=True)  #, bbox_extra_artists=(lgd,), bbox_inches='tight')
...
def firing_rates(neuront, neuroni, time, rates, name):
    ...
    plt.savefig('%s/%s/%s/%sfiring_rates%d.eps' %
                (savepath, dataname, run_no, name, timeframe),
                bbox_extra_artists=(lgd, ),
                bbox_inches='tight',
                rasterized=True)
...
def currentplots(time, exc_currents, inh_currents, timefrom, timeto, name):
    ...
    plt.tight_layout()
    lgd = plt.legend(['Exc', 'Inh'],
                     bbox_to_anchor=(1.05, 1),
                     loc=2,
                     borderaxespad=0.,
                     frameon=False,
                     framealpha=1)
    plt.savefig('%s/%s/%s/%scurrents%d.eps' %
                (savepath, dataname, run_no, name, timeto),
                bbox_extra_artists=(lgd, ),
                bbox_inches='tight')
...
def plot_xcorr(pyr1, pyr2, sst, pv, condition=None):
    ...
    plt.xticks(
        np.arange(-maxlags, maxlags + 5, 5),
        np.arange(-maxlags * binsize, maxlags * binsize + 5, 5 * binsize))
    ...
    plt.ylabel('count')
    plt.tight_layout()
    plt.savefig('%s/%s/%s/xcorr_%s.eps' %
                (savepath, dataname, run_no, condition))
...
if __name__ == "__main__":
    ...
    for i, run_no in enumerate(run_nos):
        ...
        for k in range(N_pop):
            W_sst_pv_means[i, k] = np.mean(
                W_sst_pv[i, int(k * groupsize):int((k + 1) * groupsize)])
            W_sst_pv_std[i, k] = np.std(
                W_sst_pv[i, int(k * groupsize):int((k + 1) * groupsize)])
            W_sst_pv_means_afterreward[k] = np.mean(
                W_sst_pv_afterreward[int(k * groupsize):int((k + 1) *
                                                            groupsize)])
            W_sst_pv_std_afterreward[k] = np.std(
                W_sst_pv_afterreward[int(k * groupsize):int((k + 1) *
                                                            groupsize)])
        ...
        if selfconnections == True:
            ...
            a1.axvspan((nonplasticwarmup + plasticwarmup) * 10,
                       (nonplasticwarmup + plasticwarmup + rewardsimtime) * 10,
                       facecolor='.8',
                       lw=0.0,
                       alpha=0.5)

            tsplot(a1,
                   data=PYR1and2toothers[:, :] / nS,
                   color=cmaps.viridis(1.0))  #,label='others to others')
            tsplot(a1, data=PYR0toothers[:, :] / nS,
                   color=cmaps.viridis(0.0))  #,label ='1 to others')
            tsplot(a1, data=otherstoPYR0[:, :] / nS,
                   color=cmaps.viridis(0.5))  #,label='others to 1')
            tsplot(a1, data=PYR1toPYR1[:, :] / nS,
                   color=cmaps.viridis(0.2))  #,label='others to others')
            tsplot(a1, data=PYR2toPYR2[:, :] / nS,
                   color=cmaps.viridis(0.7))  #,label='others to others')
            ...
            lgd = a1.legend([
                'others to others', '1 to others', 'others to 1', '1 to 1',
                '2 to 2'
            ],
                            bbox_to_anchor=(1.05, 1),
                            loc=2,
                            ...
                            frameon=False,
                            framealpha=1)
            plt.savefig('%s/%s/%s/weightchange.pdf' %
                        (savepath, dataname, run_no),
                        bbox_extra_artists=(lgd, ),
                        bbox_inches='tight',
                        format='pdf',
                        transparent=True)
        """Other weights"""
        """Big figure"""
        fig, ((axa1, axa3), (axa2, axa4),
              (ax6,
               ax5)) = plt.subplots(3,
                                    2,
                                    figsize=(6.3, 8),
                                    gridspec_kw={'width_ratios': [1.2, 1]})
        axa1.axvspan((nonplasticwarmup + plasticwarmup) * 10,
                     (nonplasticwarmup + plasticwarmup + rewardsimtime) * 10,
                     facecolor='.8',
                     lw=0.0,
                     alpha=0.5)
        ...
        tsplot(axa1,
               data=PYR1and2toothers[:, :] / nS,
               color=cmaps.viridis(1.0))  #,label='others to others')
        tsplot(axa1, data=PYR0toothers[:, :] / nS,
               color=cmaps.viridis(0.0))  #,label ='1 to others')
        tsplot(axa1, data=otherstoPYR0[:, :] / nS,
               color=cmaps.viridis(0.5))  #,label='others to 1')
        ...
        axa2.axvspan((nonplasticwarmup + plasticwarmup),
                     (nonplasticwarmup + plasticwarmup + rewardsimtime),
                     facecolor='.8',
                     lw=0.0,
                     alpha=0.5)
        tsplot(axa2, data=SOMPV_w[:3000, :] / nS,
               color=cmaps.viridis(0.0))  #,label='others to others')
        tsplot(axa2, data=SOMPV_w[3100:6100, :] / nS,
               color=cmaps.viridis(.33))  #,label ='1 to others')
        tsplot(axa2, data=SOMPV_w[6200:9200, :] / nS,
               color=cmaps.viridis(.66))  #,label ='1 to others')
        tsplot(axa2,
               data=SOMPV_w[9300:12300, :] / nS,
               color=cmaps.viridis(1.0))  #,label ='1 to others')
        ...
        plot_synapses(W_pyr_i,
                      W_pyr_j,
                      W_rec / gmax,
                      var_name='normalised exciatory weights',
                      plot_type='scatter',
                      cmap='viridis',
                      rasterized=True,
                      axes=axa3)
        ...
        if checker == True:
            image = plot_synapses(W_pyr_i,
                                  W_pyr_j,
                                  W_rec2 / gmax,
                                  var_name='normalised excitatory weights',
                                  plot_type='scatter',
                                  cmap='viridis',
                                  rasterized=True,
                                  axes=ax5)
        ...
        ax5.set_title('|%s=%.2f|%s=%.2f' %
                      (varied_param, dep_param[i] * 10**10, varied_param2,
                       dep_param2[i] * 10**10))

        ax6.axvspan((nonplasticwarmup + plasticwarmup) * 10,
                    (nonplasticwarmup + plasticwarmup + rewardsimtime) * 10,
                    facecolor='.8',
                    lw=0.0,
                    alpha=0.5)
        if checker == True:
            PYR1and2toothers_2 = np.concatenate(
                (PYR1toothers_2, PYR2toothers_2))

            tsplot(ax6,
                   data=PYR1and2toothers_2[:, :] / nS,
                   color=cmaps.viridis(1.0))  #,label='others to others')
            tsplot(ax6,
                   data=PYR0toothers_2[:, :] / nS,
                   color=cmaps.viridis(0.0))  #,label ='1 to others')
            tsplot(ax6,
                   data=otherstoPYR0_2[:, :] / nS,
                   color=cmaps.viridis(0.5))  #,label='others to 1')
        else:
            tsplot(ax6,
                   data=PYR1and2toothers[:, :] / nS,
                   color=cmaps.viridis(1.0))  #,label='others to others')
            tsplot(ax6, data=PYR0toothers[:, :] / nS,
                   color=cmaps.viridis(0.0))  #,label ='1 to others')
            tsplot(ax6, data=otherstoPYR0[:, :] / nS,
                   color=cmaps.viridis(0.5))  #,label='others to 1')
        ...
        lgd = plt.legend(['others to others', '1 to others', 'others to 1'],
                         bbox_to_anchor=(1.05, 1),
                         loc=2,
                         borderaxespad=0.,
                         frameon=False,
        ...
        plt.savefig('%s/%s/%s/2ndbigfigure.pdf' % (savepath, dataname, run_no),
                    bbox_extra_artists=(lgd, ),
                    bbox_inches='tight',
                    format='pdf',
                    transparent=True)
        ...
        for timeframe in [
                nonplasticwarmup, warmup + nonplasticwarmup + interval,
                warmup + rewardsimtime + halfinterval,
                warmup + rewardsimtime + interval, 60.06,
                warmup + rewardsimtime - interval - interval, 90.86, 114.8,
                total + rewardsimtime - nonplasticwarmup
        ]:

            f, (a1, a2, a3) = plt.subplots(3, sharex=True, figsize=(5, 6))
            a1.plot(PYRt[PYRi < 100] / second,
                    PYRi[PYRi < 100],
                    '.',
                    color=cmaps.viridis(0.0),
                    ms=2.0)
            a1.plot(PYRt[((PYRi >= 100) & (PYRi < 200))] / second,
                    PYRi[((PYRi >= 100) & (PYRi < 200))],
                    '.',
                    color=cmaps.viridis(0.33),
                    ms=2.0)
            a1.plot(PYRt[((PYRi >= 200) & (PYRi < 300))] / second,
                    PYRi[((PYRi >= 200) & (PYRi < 300))],
                    '.',
                    color=cmaps.viridis(0.66),
                    ms=2.0)
            a1.plot(PYRt[PYRi >= 300] / second,
                    PYRi[PYRi >= 300],
                    '.',
                    color=cmaps.viridis(1.0),
                    ms=2.0)
            ...
            a3.plot(SSTt[SSTi < 30] / second,
                    SSTi[SSTi < 30],
                    '.',
                    color=cmaps.viridis(0.0),
                    ms=2.0)
            a3.plot(SSTt[((SSTi >= 30) & (SSTi < 60))] / second,
                    SSTi[((SSTi >= 30) & (SSTi < 60))],
                    '.',
                    color=cmaps.viridis(0.33),
                    ms=2.0)
            a3.plot(SSTt[((SSTi >= 60) & (SSTi < 90))] / second,
                    SSTi[((SSTi >= 60) & (SSTi < 90))],
                    '.',
                    color=cmaps.viridis(0.66),
                    ms=2.0)
            a3.plot(SSTt[SSTi >= 90] / second,
                    SSTi[SSTi >= 90],
                    '.',
                    color=cmaps.viridis(1.0),
                    ms=2.0)
            ...
            a3.set_xticks(
                np.arange(timeframe - interval, timeframe + input_time - .01,
                          input_time))
            a3.set_title('SST')
            ...
            for a in [a1, a2, a3]:
                ...
                a.axvspan((nonplasticwarmup + plasticwarmup),
                          (nonplasticwarmup + plasticwarmup + rewardsimtime),
                          facecolor='.8',
                          lw=0.0,
                          alpha=0.5)

            plt.savefig('%s/%s/%s/bigrasterplot%s.pdf' %
                        (savepath, dataname, run_no, timeframe),
                        rasterized=True)
        ...
        if plastic == True:
            fig, ((a1, a3), (a2, a4),
                  (a6,
                   a5)) = plt.subplots(3,
                                       ...
                                       figsize=(6.3, 8),
                                       gridspec_kw={'width_ratios': [1.2, 1]})
            a1.axvspan((nonplasticwarmup + plasticwarmup),
                       (nonplasticwarmup + plasticwarmup + rewardsimtime),
                       facecolor='.8',
                       lw=0.0,
                       alpha=0.5)
            tsplot(a1, data=PVPYR_w[:12000, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            tsplot(a1,
                   data=PVPYR_w[12000:24000, :] / nS,
                   color=cmaps.viridis(.33))  #,label ='1 to others')
            tsplot(a1,
                   data=PVPYR_w[24000:36000, :] / nS,
                   color=cmaps.viridis(.66))  #,label ='1 to others')
            tsplot(a1,
                   data=PVPYR_w[36000:48000, :] / nS,
                   color=cmaps.viridis(1.0))  #,label ='1 to others')
            ...
            a2.axvspan((nonplasticwarmup + plasticwarmup),
                       (nonplasticwarmup + plasticwarmup + rewardsimtime),
                       facecolor='.8',
                       lw=0.0,
                       alpha=0.5)
            tsplot(a2, data=PYRPV_w[:8463, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            tsplot(a2,
                   data=PYRPV_w[11284:19747, :] / nS,
                   color=cmaps.viridis(.33))  #,label ='1 to others')
            tsplot(a2,
                   data=PYRPV_w[22568:31031, :] / nS,
                   color=cmaps.viridis(.66))  #,label ='1 to others')
            tsplot(a2,
                   data=PYRPV_w[33852:42315, :] / nS,
                   color=cmaps.viridis(1.0))  #,label ='1 to others')
            ...
            a3.axvspan((nonplasticwarmup + plasticwarmup),
                       (nonplasticwarmup + plasticwarmup + rewardsimtime),
                       facecolor='.8',
                       lw=0.0,
                       alpha=0.5)
            tsplot(a3, data=VIPSOM_w[:1500, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            tsplot(a3,
                   data=VIPSOM_w[1500:3000, :] / nS,
                   color=cmaps.viridis(.33))  #,label ='1 to others')
            tsplot(a3,
                   data=VIPSOM_w[3000:4500, :] / nS,
                   color=cmaps.viridis(.66))  #,label ='1 to others')
            tsplot(a3,
                   data=VIPSOM_w[4500:6000, :] / nS,
                   color=cmaps.viridis(1.0))  #,label ='1 to others')
            ...
            a4.axvspan((nonplasticwarmup + plasticwarmup),
                       (nonplasticwarmup + plasticwarmup + rewardsimtime),
                       facecolor='.8',
                       lw=0.0,
                       alpha=0.5)
            tsplot(a4, data=SOMVIP_w[:1500, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            tsplot(a4,
                   data=SOMVIP_w[1500:3000, :] / nS,
                   color=cmaps.viridis(.33))  #,label ='1 to others')
            tsplot(a4,
                   data=SOMVIP_w[3000:4500, :] / nS,
                   color=cmaps.viridis(.66))  #,label ='1 to others')
            tsplot(a4,
                   data=SOMVIP_w[4500:6000, :] / nS,
                   color=cmaps.viridis(1.0))  #,label ='1 to others')
            ...
            a5.axvspan((nonplasticwarmup + plasticwarmup),
                       (nonplasticwarmup + plasticwarmup + rewardsimtime),
                       facecolor='.8',
                       lw=0.0,
                       alpha=0.5)
            tsplot(a5, data=PYRVIP_w[:5000, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            tsplot(a5,
                   data=PYRVIP_w[5000:10000, :] / nS,
                   color=cmaps.viridis(.33))  #,label ='1 to others')
            tsplot(a5,
                   data=PYRVIP_w[10000:15000, :] / nS,
                   color=cmaps.viridis(.66))  #,label ='1 to others')
            tsplot(a5,
                   data=PYRVIP_w[15000:20000, :] / nS,
                   color=cmaps.viridis(1.0))  #,label ='1 to others')
            ...
            a6.axvspan((nonplasticwarmup + plasticwarmup),
                       (nonplasticwarmup + plasticwarmup + rewardsimtime),
                       facecolor='.8',
                       lw=0.0,
                       alpha=0.5)
            tsplot(a6, data=PYRSOM1_w[:, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            tsplot(a6, data=PYRSOM2_w[:, :] / nS,
                   color=cmaps.viridis(.33))  #,label ='1 to others')
            tsplot(a6, data=PYRSOM3_w[:, :] / nS,
                   color=cmaps.viridis(.66))  #,label ='1 to others')
            tsplot(a6, data=PYRSOM4_w[:, :] / nS,
                   color=cmaps.viridis(1.0))  #,label ='1 to others')
            ...
            plt.tight_layout()
            plt.savefig('%s/%s/%s/otherweights.pdf' %
                        (savepath, dataname, run_no),
                        bbox_extra_artists=(lgd, ),
                        bbox_inches='tight',
                        format='pdf',
            ...
            a1.axvspan((nonplasticwarmup + plasticwarmup),
                       ...
                       lw=0.0,
                       alpha=0.5)
            tsplot(a1, data=SOMPYR_w[:12000, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            tsplot(a1,
                   data=SOMPYR_w[12000:24000, :] / nS,
                   color=cmaps.viridis(.33))  #,label ='1 to others')
            tsplot(a1,
                   data=SOMPYR_w[24000:36000, :] / nS,
                   color=cmaps.viridis(.66))  #,label ='1 to others')
            tsplot(a1,
                   data=SOMPYR_w[36000:48000, :] / nS,
                   color=cmaps.viridis(1.0))  #,label ='1 to others')
            ...
            a2.axvspan((nonplasticwarmup + plasticwarmup),
                       ...
                       lw=0.0,
                       alpha=0.5)
            tsplot(a2, data=PVVIP_w[:1500, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            tsplot(a2,
                   data=PVVIP_w[1500:3000, :] / nS,
                   color=cmaps.viridis(.33))  #,label ='1 to others')
            tsplot(a2,
                   data=PVVIP_w[3000:4500, :] / nS,
                   color=cmaps.viridis(.66))  #,label ='1 to others')
            tsplot(a2,
                   data=PVVIP_w[4500:6000, :] / nS,
                   color=cmaps.viridis(1.0))  #,label ='1 to others')
            ...
            a3.axvspan((nonplasticwarmup + plasticwarmup),
                       ...
                       lw=0.0,
                       alpha=0.5)
            tsplot(a3, data=PVPV_w[:3600, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            tsplot(a3,
                   data=PVPV_w[3600:7200, :] / nS,
                   color=cmaps.viridis(.33))  #,label ='1 to others')
            tsplot(a3,
                   data=PVPV_w[7200:10800, :] / nS,
                   color=cmaps.viridis(.66))  #,label ='1 to others')
            tsplot(a3,
                   data=PVPV_w[10800:14400, :] / nS,
                   color=cmaps.viridis(1.0))  #,label ='1 to others')
            ...
            a4.axvspan((nonplasticwarmup + plasticwarmup),
                       ...
                       lw=0.0,
                       alpha=0.5)
            tsplot(a4, data=SOMSOM_w[:3600, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            tsplot(a4,
                   data=SOMSOM_w[3600:7200, :] / nS,
                   color=cmaps.viridis(.33))  #,label ='1 to others')
            tsplot(a4,
                   data=SOMSOM_w[7200:10800, :] / nS,
                   color=cmaps.viridis(.66))  #,label ='1 to others')
            tsplot(a4,
                   data=SOMSOM_w[10800:14400, :] / nS,
                   color=cmaps.viridis(1.0))  #,label ='1 to others')
            ...
            a5.axvspan((nonplasticwarmup + plasticwarmup),
                       ...
                       lw=0.0,
                       alpha=0.5)
            tsplot(a5, data=VIPPYR_w[:5000, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            tsplot(a5,
                   data=VIPPYR_w[5000:10000, :] / nS,
                   color=cmaps.viridis(.33))  #,label ='1 to others')
            tsplot(a5,
                   data=VIPPYR_w[10000:15000, :] / nS,
                   color=cmaps.viridis(.66))  #,label ='1 to others')
            tsplot(a5,
                   data=VIPPYR_w[15000:20000, :] / nS,
                   color=cmaps.viridis(1.0))  #,label ='1 to others')
            ...
            a6.axvspan((nonplasticwarmup + plasticwarmup),
                       ...
                       lw=0.0,
                       alpha=0.5)
            tsplot(a6, data=PVSOM_w[:3600, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            tsplot(a6,
                   data=PVSOM_w[3600:7200, :] / nS,
                   color=cmaps.viridis(.33))  #,label ='1 to others')
            tsplot(a6,
                   data=PVSOM_w[7200:10800, :] / nS,
                   color=cmaps.viridis(.66))  #,label ='1 to others')
            tsplot(a6,
                   data=PVSOM_w[10800:14400, :] / nS,
                   color=cmaps.viridis(1.0))  #,label ='1 to others')
            ...
            a7.axvspan((nonplasticwarmup + plasticwarmup),
                       (nonplasticwarmup + plasticwarmup + rewardsimtime),
                       facecolor='.8',
                       lw=0.0,
                       alpha=0.5)
            tsplot(a7, data=VIPPV_w[:1500, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            tsplot(a7,
                   data=VIPPV_w[1500:3000, :] / nS,
                   color=cmaps.viridis(.33))  #,label ='1 to others')
            tsplot(a7,
                   data=VIPPV_w[3000:4500, :] / nS,
                   color=cmaps.viridis(.66))  #,label ='1 to others')
            tsplot(a7,
                   data=VIPPV_w[4500:6000, :] / nS,
                   color=cmaps.viridis(1.0))  #,label ='1 to others')
            ...
            a8.axvspan((nonplasticwarmup + plasticwarmup),
                       (nonplasticwarmup + plasticwarmup + rewardsimtime),
                       facecolor='.8',
                       lw=0.0,
                       alpha=0.5)
            tsplot(a8, data=VIPVIP_w[:, :] / nS,
                   color=cmaps.viridis(0.0))  #,label='others to others')
            ...
            plt.tight_layout()
            plt.savefig('%s/%s/%s/otherweights2.pdf' %
                        (savepath, dataname, run_no),
                        bbox_extra_artists=(lgd, ),
                        bbox_inches='tight',
                        format='pdf',
        ...
        plot_tuningcurves(tunings_initial,
                          tuning_rewardend,
                          tuning_after_rewardend,
                          tunings_final,
                          N_pyr,
                          N_pop,
                          save='%s/%s/%s' % (savepath, dataname, run_no),
                          name='')
        plot_tuningcurves(SOMtunings_initial,
                          SOMtuning_rewardend,
                          SOMtuning_after_rewardend,
                          SOMtunings_final,
                          N_sst,
                          N_pop,
                          save='%s/%s/%s' % (savepath, dataname, run_no),
                          name='SST')
        plot_tuningcurves(PVtunings_initial,
                          PVtuning_rewardend,
                          PVtuning_after_rewardend,
                          PVtunings_final,
                          N_pv,
                          1,
                          save='%s/%s/%s' % (savepath, dataname, run_no),
        ...
        plot_responsechangehistogram(tunings_initial,
                                     tunings_final,
                                     N_pyr,
                                     N_pop,
                                     None,
        ...
        axa2.axvspan((nonplasticwarmup + plasticwarmup),
                     ...
                     lw=0.0,
                     alpha=0.5)
        tsplot(axa2, data=SOMPV_w[:3600, :] / nS,
               color=cmaps.viridis(0.0))  #,label='others to others')
        tsplot(axa2, data=SOMPV_w[3600:7200, :] / nS,
               color=cmaps.viridis(.33))  #,label ='1 to others')
        tsplot(axa2,
               data=SOMPV_w[7200:10800, :] / nS,
               color=cmaps.viridis(.66))  #,label ='1 to others')
        tsplot(axa2,
               data=SOMPV_w[10800:14400, :] / nS,
               color=cmaps.viridis(1.0))  #,label ='1 to others')
        ...
        axa2.set_xticks(
            np.arange(0, (nonplasticwarmup + plasticwarmup + rewardsimtime),
                      10))
        ...
        plot_synapses(W_pyr_i,
                      W_pyr_j,
                      W_rec_afterwarmup * siemens / gmax,
                      var_name='normalised excitatory weights',
                      plot_type='scatter',
                      cmap='viridis',
                      rasterized=True,
        ...
        plot_synapses(W_pyr_i,
                      W_pyr_j,
                      W_rec_afterreward * siemens / gmax,
                      var_name='normalised excitatory weights',
                      plot_type='scatter',
                      cmap='viridis',
                      rasterized=True,
        ...
        axa6.errorbar(ind,
                      W_sst_pv_means_afterreward[:] / nS,
                      yerr=W_sst_pv_std_afterreward[:] / nS,
                      color='.75',
                      ls=' ',
                      marker='o',
                      capsize=5,
        ...
        plt.savefig('%s/%s/%s/2ndAfterreward.eps' %
                    (savepath, dataname, run_no),
                    bbox_extra_artists=(lgd, ),
                    bbox_inches='tight',
                    format='pdf',
        ...
        axa1.set_xlabel('time [seconds]')
        axa1.set_ylabel('excitatory weights [nS]')
        axa1.set_xlim((nonplasticwarmup + plasticwarmup + rewardsimtime) * 10,
                      total * 10)
        axa1.set_xticks(
            np.arange(
                (nonplasticwarmup + plasticwarmup + rewardsimtime + 2.1) * 10,
                total * 10, 100))
        ...
        axa2.set_ylabel('SST-to-PV weights [nS]')
        axa2.set_ylim(-.02, 1.02)
        axa2.set_xlim((nonplasticwarmup + plasticwarmup + rewardsimtime),
                      total)
        axa2.set_xticks(
            np.arange((nonplasticwarmup + plasticwarmup + rewardsimtime + 2.1),
                      total, 10))
        ...
        plt.savefig('%s/%s/%s/2ndFinal.eps' % (savepath, dataname, run_no),
                    bbox_extra_artists=(lgd, ),
                    bbox_inches='tight',
                    format='pdf',
                    transparent=True)
        ...
        tuning_before, tuning_after = get_pop_tuning(tunings_before[:, :4],
                                                     tunings_after[:, :4])
        tuning_initial[i, :, :], tuning_final[i, :, :] = get_pop_tuning(
            tunings_initial[:, :4], tunings_final[:, :4])
        SSTtuning_initial[i, :, :], SSTtuning_final[i, :, :] = get_pop_tuning(
            SOMtunings_initial[:, :4], SOMtunings_final[:, :4])
        PVtuning_initial[i, :, :], PVtuning_final[i, :, :] = get_pop_tuning(
            PVtunings_initial[:, :4], PVtunings_final[:, :4])
        ...
        plot_tuningcurves(tunings_initial[:, :4],
                          None,
                          None,
                          tunings_final[:, :4],
                          N_pyr,
        ...
        plot_tuningcurves(tunings_initial,
                          ...
                          name='warmup')
        if SOMswitch == True:
            plot_tuningcurves(tunings_final[:, :4],
                              None,
                              None,
                              tuning_finalSOMswitch[:, :4],
                              N_pyr,
                              N_pop,
                              save='%s/%s/%s' % (savepath, dataname, run_no),
                              name='finalSOMswitch')
            plot_SOMswitch(tunings_final[:, :4],
                           tuning_finalSOMswitch[:, :4],
                           N_pyr,
                           N_pop,
                           save='%s/%s/%s' % (savepath, dataname, run_no),
        ...
        plot_alltuningcurves(tunings_initial[:, :4],
                             None,
                             None,
                             tunings_final[:, :4],
                             N_pyr,
        ...
        plot_alltuningcurves(SOMtunings_initial,
                             None,
                             None,
                             SOMtunings_final,
                             N_sst,
        ...
        plot_alltuningcurves(PVtunings_initial,
                             None,
                             None,
                             PVtunings_final,
                             N_pv,
        ...
        #Big figure
        fig, ((axa1, axa3), (axa2, axa4)) = plt.subplots(2,
                                                         2)  #,figsize=(15,15))
        axa1.axvspan((nonplasticwarmup + plasticwarmup) * 10,
                     (nonplasticwarmup + plasticwarmup + rewardsimtime) * 10,
                     facecolor='.8',
        ...
        plt.tight_layout()
        plt.savefig('%s/%s/%s/bigfigure.eps' % (savepath, dataname, run_no),
                    bbox_extra_artists=(lgd, ),
                    bbox_inches='tight',
                    format='pdf',
                    transparent=True)
        ...
        if xcorr == True:
            ...
            rewardend = int(1000 *
                            (nonplasticwarmup + plasticwarmup + rewardsimtime))
            end = int(1000 * (nonplasticwarmup + plasticwarmup +
                              rewardsimtime + norewardsimtime))
            sst_reward, binssst, patches = plt.hist(SSTtrain,
                                                    bins=range(
                                                        rewardstart, rewardend,
                                                        binsize))
            pv_reward, binspv, patches = plt.hist(PVtrain,
                                                  bins=range(
                                                      rewardstart, rewardend,
                                                      binsize))
            pyr1_reward, binspyr1, patches = plt.hist(PYR1train,
                                                      bins=range(
                                                          rewardstart,
                                                          rewardend, binsize))
            pyr2_reward, binspyr2, patches = plt.hist(PYR2train,
                                                      bins=range(
                                                          rewardstart,
                                                          rewardend, binsize))
            pyr3_reward, binspyr3, patches = plt.hist(PYR3train,
                                                      bins=range(
                                                          rewardstart,
                                                          rewardend, binsize))
            ...
            pyr1_reward1, binspyr2, patches = plt.hist(PYR1train1,
                                                       bins=range(
                                                           rewardstart,
                                                           rewardend, binsize))
            pyr2_reward1, binspyr2, patches = plt.hist(PYR2train1,
                                                       bins=range(
                                                           rewardstart,
                                                           rewardend, binsize))
            pyr3_reward1, binspyr3, patches = plt.hist(PYR3train1,
                                                       bins=range(
                                                           rewardstart,
                                                           rewardend, binsize))
            sst_reward1, binssst, patches = plt.hist(SSTtrain1,
                                                     bins=range(
                                                         rewardstart,
                                                         rewardend, binsize))
            pv_reward1, binspv, patches = plt.hist(PVtrain1,
                                                   bins=range(
                                                       rewardstart, rewardend,
                                                       binsize))
            ...
            sst_afterreward, binssst, patches = plt.hist(SSTtrain1,
                                                         bins=range(
                                                             rewardend, end,
                                                             binsize))
            pv_afterreward, binspv, patches = plt.hist(PVtrain1,
                                                       bins=range(
                                                           rewardend, end,
                                                           binsize))
            pyr1_afterreward, binspyr1, patches = plt.hist(PYR1train1,
                                                           bins=range(
                                                               rewardend, end,
                                                               binsize))
            pyr2_afterreward, binspyr2, patches = plt.hist(PYR2train1,
                                                           bins=range(
                                                               rewardend, end,
                                                               binsize))
            pyr3_afterreward, binspyr3, patches = plt.hist(PYR3train1,
                                                           bins=range(
                                                               rewardend, end,
                                                               binsize))
            ...
            sst_afterreward1, binssst, patches = plt.hist(SSTtrain1,
                                                          bins=range(
                                                              rewardend, end,
                                                              binsize))
            pv_afterreward1, binspv, patches = plt.hist(PVtrain1,
                                                        bins=range(
                                                            rewardend, end,
                                                            binsize))
            pyr1_afterreward1, binspyr1, patches = plt.hist(PYR1train1,
                                                            bins=range(
                                                                rewardend, end,
                                                                binsize))
            pyr2_afterreward1, binspyr2, patches = plt.hist(PYR2train1,
                                                            bins=range(
                                                                rewardend, end,
                                                                binsize))
            pyr3_afterreward1, binspyr3, patches = plt.hist(PYR3train1,
                                                            bins=range(
                                                                rewardend, end,
                                                                binsize))
            ...
            plot_xcorr(pyr1_reward, pyr3_reward, sst_reward, pv_reward,
                       'reward')
            plot_xcorr2(pyr1_reward, pyr3_reward, sst_reward, pv_reward,
                        'reward2')
            plot_xcorr(pyr1_afterreward, pyr3_afterreward, sst_afterreward,
                       pv_afterreward, 'afterreward')
            plot_xcorr2(pyr1_afterreward, pyr3_afterreward, sst_afterreward,
                        pv_afterreward, 'afterreward2')

    # plot excitatory structure index as a function of varied_param
    plt.figure(figsize=(3.5, 3))
    for pa in np.unique(dep_param2):
        plt.plot(dep_param[dep_param2 == pa] / nS,
                 impact_afterreward[dep_param2 == pa],
                 '.',
                 markersize=10,
                 label=str(pa))  #, c=performance_binary)
    ...
    plt.savefig('%s/%s/%s/impact_afterreward_colored.eps' %
                (savepath, dataname, run_no),
                bbox_extra_artists=(lgd, ),
                bbox_inches='tight',
                format='pdf',
    ...
    plt.savefig('%s/%s/%s/impact_colored.eps' % (savepath, dataname, run_no),
                bbox_extra_artists=(lgd, ),
                bbox_inches='tight',
                format='pdf',
                transparent=True)
    ...
    for pa in np.unique(dep_param):
        plt.plot(dep_param2[dep_param == pa] / nS,
                 impact_afterreward[dep_param == pa],
                 '.',
                 markersize=10,
                 label=str(pa))  #, c=performance_binary)
    plt.ylabel('Excitatory structure index')
    ...
    plt.savefig('%s/%s/%s/impact_afterrewardp2_colored.eps' %
                (savepath, dataname, run_no),
                bbox_extra_artists=(lgd, ),
                bbox_inches='tight',
                format='pdf',
    ...
    plt.savefig('%s/%s/%s/impactp2_colored.eps' % (savepath, dataname, run_no),
                bbox_extra_artists=(lgd, ),
                bbox_inches='tight',
                format='pdf',
                transparent=True)
    ...
    plt.figure(figsize=(3.5, 3))
    plt.plot(dep_param / nS, impact, '.', color='k',
             markersize=5)  #, c=performance_binary)
    plt.ylabel('Excitatory structure index')
    plt.xlabel('SST-to-PYR synaptic strength [nS]')

    plt.savefig('%s/%s/%s/impact.eps' % (savepath, dataname, run_no),
                bbox_extra_artists=(lgd, ),
                bbox_inches='tight',
                format='pdf',
                transparent=True)
    ...
    plt.figure(figsize=(3.5, 3))
    plt.plot(dep_param[(dep_param / 10**(-9)) > 0.3] / nS,
             impact_afterreward[(dep_param / 10**(-9)) > 0.3],
             '.',
             color='k',
             markersize=10)
    plt.ylabel('Excitatory structure index')
    plt.xlabel('SST-to-PC synaptic strength [nS]')
    plt.savefig('%s/%s/%s/impact_afterreward.eps' %
                (savepath, dataname, run_no),
                bbox_extra_artists=(lgd, ),
                bbox_inches='tight',
                format='pdf',
    ...
    plt.figure(figsize=(3.5, 3))
    plt.plot(dep_param / nS, impactmax, '.', color='k',
             markersize=5)  #, c=performance_binary)
    plt.ylabel('Excitatory structure index')
    plt.xlabel('SST-to-PYR synaptic strength [nS]')

    plt.savefig('%s/%s/%s/impactmax.eps' % (savepath, dataname, run_no),
                bbox_extra_artists=(lgd, ),
                bbox_inches='tight',
                format='pdf',
                transparent=True)
    ...
    plt.figure(figsize=(3.5, 3))
    plt.plot(dep_param / nS,
             impactmax_afterreward,
             '.',
             color='k',
             markersize=5)  #, c=performance_binary)
    ...
    plt.ylabel('Excitatory structure index')
    plt.xlabel('SST-to-PYR synaptic strength [nS]')
    plt.savefig('%s/%s/%s/impactmax_afterreward.eps' %
                (savepath, dataname, run_no),
                bbox_extra_artists=(lgd, ),
                bbox_inches='tight',
                format='pdf',
    ...
    for c1, par1 in enumerate(param_range):
        for c2, par2 in enumerate(param_range2):
            try:
                impact_m[c1, c2] = impact[((dep_param == par1) &
                                           (dep_param2 == par2))]
                sst_pv_m[c1, c2] = sst_pv[((dep_param == par1) &
                                           (dep_param2 == par2))]
                tuning_m[c1, c2] = tuning[((dep_param == par1) &
                                           (dep_param2 == par2))]
    ...
    for key in measures:
        plt.figure()
        ax = subplot(111)
        plt.imshow(measures[key], interpolation='nearest',
                   origin='lower')  #, c=performance_binary)
        plt.xlabel(varied_param2)
        ...
        plt.savefig('%s/%s/%s/%s3D_%s.eps' %
                    (savepath, dataname, run_no, key, varied_param))

    fig, ax = plt.subplots(figsize=(5, 4))
    ax.errorbar(ind,
                np.mean(W_sst_pv_means, 0) / nS,
                yerr=np.mean(W_sst_pv_std, 0) / nS,
                color='0.75',
                ls=' ',
                marker='o',
                capsize=5,
    ...
    plt.savefig('%s/%s/%s/SOMPV_dist_grandavg.eps' %
                (savepath, dataname, run_no))

    plot_grandavgtuningcurves(tuning_initial,
                              tuning_final,
                              N_pop,
                              save='%s/%s/%s' % (savepath, dataname, run_no),
                              name='')
    plot_grandavgtuningcurves(SSTtuning_initial,
                              SSTtuning_final,
                              N_pop,
                              save='%s/%s/%s' % (savepath, dataname, run_no),
                              name='SST')
    plot_grandavgtuningcurves(PVtuning_initial,
                              PVtuning_final,
                              1,
                              save='%s/%s/%s' % (savepath, dataname, run_no),
                              name='PV')
