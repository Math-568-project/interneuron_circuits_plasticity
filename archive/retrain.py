...
def run_network(params):
    ...
    mona = StateMonitor(
        ...
        'w',
        record=con_REC[
            '(i >= 100) and (i < 200) and ((j < 100) or (j >= 200))'],
        dt=100 * ms)  # pyr 1 to others
    monb = StateMonitor(
        con_REC,
        'w',
        record=con_REC[
            '(j >= 100) and (j < 200) and ((i < 100) or (i >= 200))'],
        dt=100 * ms)  # other to pyr 1

    monc = StateMonitor(con_REC,
                        'w',
                        record=con_REC[0:100, 200:400],
                        dt=100 * ms)  # pyr0 to others
    mond = StateMonitor(
        con_REC,
        'w',
        record=con_REC[
            '(i >= 200) and (i < 300) and ((j < 100) or (j >= 300))'],
        dt=100 * ms)  # pyr 2 to others
    mone = StateMonitor(
        con_REC,
        'w',
        record=con_REC[
            '(i < 400) and (i >= 300) and ((j < 100) or ((j >= 200) and (j < 300)))'],
    ...
    # Top down input: reward for stimulus 0
    ...
    _, impactmax_afterreward = calc_impact(con_REC.w)
    ...
    results_file = RESULTS_DIR + f'/retrain_results_tuned{TUNED_ORI}_1.pkl'
    ...
    results_file = f'./results/retrain_results_tuned{TUNED_ORI}_2.pkl'
    print("Saving results to:", results_file)
    with open(results_file, 'wb') as f:
        pickle.dump(results, f)