...
class ExperimentReader(object):
    ...
    def get_all_experiment_runs(self):
        ...
        for fname in self._files_in_rundir:
            ...
            if os.path.basename(fname)[0] != '_':
                ...
                results[exp_id] = {
                    'run': self.get_experiment_run(exp_id),
    ...
    def get_experiment_config(self, exp_id):
        with open(os.path.join(self._run_dir, str(exp_id),
                               'config.json')) as f:
    ...
    def get_artifact_file_names(self, exp_id, run_dict=None):
        ...
        return [
            os.path.join(base_dir, fname) for fname in run_dict['artifacts']
        ]
    ...
    def try_loading_artifacts(self, exp_id, run_dict=None):
        ...
        for fname in fnames:
            ...
            if extension in {'.pkl', '.pickle'}:
                ...
                raise ValueError(
                    "I have no clue how to load file {} with extension {}".
...
def get_particular_stimulus_times(stimuli_orientation,
                                  stimuli_t,
                                  no_stimuli,
                                  upto=None,
                                  startat=None):
    ...
    for i in range(no_stimuli):
        ...
        if startat != None:
            if startat < stimuli_t[-1]:
                ...
                raise ValueError(
                    'startat is not supposed to be at the end of the simulation'
                )
        last[i] = stimuli_t[stimuli == i][-1]
...
def get_spiketrains_foreachstim(spike_train,
                                stimuli_orientation,
                                stimuli_t,
                                no_stimuli,
                                input_time,
    ...
    stimulus_seq = get_stimulus_sequence(stimuli_orientation, stimuli_t, upto,
                                         input_time)
    ...
    for i in range(0, no_stimuli):
        ...
        for k in range(N_neurons):
            ...
            for idx in indices:
                ...
                spike_trains.append(
                    (spike_train[k][(spike_train[k] > idx * input_time) & (
                        spike_train[k] < idx * input_time + stim_time)]))
            Neuron[str(k)] = spike_trains
        Data[str(i)] = Neuron
    return Data


def get_tuning_avgoverperiod(spike_train,
                             stimuli_orientation,
                             stimuli_t,
                             no_stimuli,
                             input_time,
    ...
    for i in range(0, no_stimuli):
        ...
        for k in range(N_neurons):
            ...
            for idx in indices:
                # get spike counts of neuron k that happen during stimulus with index idx
                spikes.append(
                    np.sum(((spike_train[k] >= idx * input_time) &
                            (spike_train[k] <
                             (idx * input_time) + stim_time))))
...
def get_currentratio_foreachstim(currents,
                                 stimuli_orientation,
                                 stimuli_t,
                                 no_stimuli,
                                 input_time,
    ...
    for i in range(0, no_stimuli):
        # get all indices of stimulus:
        indices = np.nonzero(
            stimulus_seq == orientations[i]
        )[0]  # get first index of orientation that matches desired orientation
        #print(indices)
        # get only indices of stimuli that occur after time startat:
        indices = indices[indices >= startat_idx]
        #print(indices)
        ...
        for k in range(N_neurons):
            ...
            for idx in indices:
                ...
                currentEcurrent = currents.IsynE[k][
                    (currents.t >= idx * input_time)
                    & (currents.t < idx * input_time + stim_time)]
                currentIcurrent = currents.IsynI[k][
                    (currents.t >= idx * input_time)
                    & (currents.t < idx * input_time + stim_time)]
            ...
            ampE[k, i] = np.max(
                np.mean(Ecurrent,
                        0))  # take mean over list elements to get mean current
...
def get_spike_response(spike_train,
                       no_stimuli,
                       stim_change_time,
                       first=None,
                       last=None):
    ...
    for k in range(N_neurons):
        for i in range(no_stimuli):
            if not first is None:
                response[k, i] = np.sum(
                    ((spike_train[k] > first[i] * second) &
                     (spike_train[k] < first[i] * second + stim_change_time)))
            elif not last is None:
                response[k, i] = np.sum(
                    ((spike_train[k] < last[i] * second) &
                     (spike_train[k] > last[i] * second - stim_change_time)))
...
def get_firingrate(spike_train, starttime, endtime):
    ...
    for k in range(N_neurons):
        firing_rate[k] = np.sum(
            ((spike_train[k] > starttime) &
             (spike_train[k] < endtime))) / (endtime - starttime)
    return firing_rate


def get_tuning(spike_train, stimuli_orientation, stimuli_t, no_stimuli):
    N_neurons = len(spike_train)
    first, last = get_stimulus_times(stimuli_orientation, stimuli_t,
                                     no_stimuli)
    ...
    for k in range(N_neurons):
        for i in range(no_stimuli):
            first_response[k, i] = np.sum(
                ((spike_train[k] > first[i] * second) &
                 (spike_train[k] < first[i] * second + 50 * ms)))
            last_response[k, i] = np.sum(
                ((spike_train[k] < last[i] * second) &
                 (spike_train[k] > last[i] * second - 50 * ms)))
...
def plot_tuning(
    ...
    if avg:
        # plot average
        plt.plot([0, 1, 2, 3], [
            mean(first_response[:, 0]),
            mean(first_response[:, 1]),
            mean(first_response[:, 2]),
            mean(first_response[:, 3])
        ], 'k')
        plt.plot([0, 1, 2, 3], [
            mean(last_response[:, 0]),
            mean(last_response[:, 1]),
            mean(last_response[:, 2]),
            mean(last_response[:, 3])
        ], 'r')
        ...
        plt.plot([0, 1, 2, 3], [
            first_response[:, 0], first_response[:, 1], first_response[:, 2],
            first_response[:, 3]
        ], 'k')
        plt.plot([0, 1, 2, 3], [
            last_response[:, 0], last_response[:, 1], last_response[:, 2],
            last_response[:, 3]
        ], 'r')
        plt.xlabel('orientation')
        plt.xlabel('firing rate')
...
def get_pop_tuning(first_response, last_response):
    meanfirst = np.zeros(
        (np.shape(first_response)[1], np.shape(first_response)[1]))
    meanlast = np.zeros(
        (np.shape(first_response)[1], np.shape(first_response)[1]))
    for i in range(np.shape(first_response)[1]):
        meanfirst[i, :] = np.mean(first_response[i * 100:(i * 100) + 100, :],
                                  0)
    ...
    if avg:
        ...
        for i in range(0, NPYR, 100):
            ...
            plt.plot(np.arange(4), [
                mean(first_response[i:i + 100, 0]),
                mean(first_response[i:i + 100, 1]),
                mean(first_response[i:i + 100, 2]),
                mean(first_response[i:i + 100, 3])
            ], 'k')
            plt.plot(np.arange(4), [
                mean(last_response[i:i + 100, 0]),
                mean(last_response[i:i + 100, 1]),
                mean(last_response[i:i + 100, 2]),
                mean(last_response[i:i + 100, 3])
            ],
        ...
        for i in range(0, NPYR, 100):
            ...
            plt.plot([0, 1, 2, 3], [
                first_response[i:i + 100, 0], first_response[i:i + 100, 1],
                first_response[i:i + 100, 2], first_response[i:i + 100, 3]
            ], 'k')
            plt.plot([0, 1, 2, 3], [
                last_response[i:i + 100, 0], last_response[i:i + 100, 1],
                last_response[i:i + 100, 2], last_response[i:i + 100, 3]
            ], 'r')
            plt.xlabel('orientation')
            plt.title('population %d' % ((i / 100) + 1))
            plt.tight_layout()
...
def get_response_towards_rewarded(tuning_curves, prev_tuning_curves):
    ...
    for i in range(len(tuning_curves)):
        response[i] = (tuning_curves[i][0] -
                       prev_tuning_curves[i][0]) / prev_tuning_curves[0][0]
    return response
...
def plot_weights(neuron_i, neuron_j, weights, title):
    ax = plot_synapses(neuron_i,
                       neuron_j,
                       weights,
                       var_name='synaptic weights',
                       plot_type='scatter',
                       cmap='hot')
...
def calculate_response_increase(tuning_before, tuning_after, N_pyr):
    ...
    for k in range(N_pyr):
        baseline_tuning_after = np.mean(
            tuning_after[k, (np.arange(len(tuning_after)) != k) &
                         (np.arange(len(tuning_after)) != 0)])
        baseline_tuning_before = np.mean(
            tuning_before[k, (np.arange(len(tuning_after)) != k) &
                          (np.arange(len(tuning_after)) != 0)])

        response_rel_to_max[k] = (tuning_after[k, 0] - baseline_tuning_after
                                  ) / (tuning_after[k, k] -
                                       baseline_tuning_after)
        response_rel_to_max_before[k] = (
            tuning_before[k, 0] - baseline_tuning_before) / (
                tuning_before[k, k] - baseline_tuning_before)
...
if __name__ == "__main__":
    ...
    first, last = get_tuning(spike_train,
                             stimuli_orientation,
                             stimuli_t,
                             no_stimuli=4)
    plot_PYRtuning(first, last)

    plot_weights(recurrent_i, recurrent_j, recurrent_weights,
                 'recurrent connections')
    plot_weights(PYR2VIP_i, PYR2VIP_j, PYR2VIP_weights,
                 'PYR to VIP connections')
